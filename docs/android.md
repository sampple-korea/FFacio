...
- Enrollment rejects near-duplicate samples, requires pose diversity, checks final template cohesion before saving, and blocks faces that already match an enrolled user.
- During enrollment, the camera preview shows center/left/right pose progress plus sample progress so users know when to slowly turn their face for a stronger template.
