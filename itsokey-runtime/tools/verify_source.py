...
for endpoint in ['/api/oauth/authorize.do', '/api/oauth/me.do', '/api/widget/oauth/generated.do', '/api/widget/devices.do', '/api/widget/oauth/refresh.do', '/api/widget/device/', '/control.do']:
...
require('loginWithCredentials(email, password)' in java, "Native credential login call is missing")
require('.put("type", "EMAIL")' in java, "Official EMAIL authorization payload is missing")
require('TYPE_TEXT_VARIATION_PASSWORD' in java, "Native password input is missing")
require('WebView' not in java and 'localStorage' not in java and 'fromWebStorage' not in java, "Legacy browser token scraping remains")
require('sessionStore.save(widgetSession)' in java, "Widget session is not persisted")
...
if errors:
    ...
    for error in errors:
        print("-", error)
    sys.exit(1)
print("PASS runtime source checks=29")
