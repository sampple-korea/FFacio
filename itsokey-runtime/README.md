...
- `:app` — 독립 실행 관리 앱 + 이메일 직접 로그인 + 암호화 세션 저장 + Binder 서비스
...
- 공식 앱과 동일한 `EMAIL` 인증 API를 통한 이메일·비밀번호 로그인
- 회원정보 확인 후 전용 위젯 세션 발급
- 브라우저 기반 소셜 로그인과 토큰 추출 코드 제거
- 비밀번호 비저장
...
- 실제 ITSOKEY 계정·HG-1300 도어락이 필요한 로그인과 실문 OPEN은 별도 실기기 검증이 필요합니다.
